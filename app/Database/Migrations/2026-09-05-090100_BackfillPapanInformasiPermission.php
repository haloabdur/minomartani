...
/**
 * Papan Informasi is a new per-user menu permission ('menu.papan_informasi',
 * 'admin'-group only - see Config\AuthGroups + Admin\Users). Grant it to
 * every account currently in the 'admin' group, mirroring
 * BackfillAdminMenuPermissions, so existing RT admins don't silently lose
 * access the moment this deploys.
 *
 * Idempotent: skips users that already have the permission, safe to
 * re-run.
 */
class BackfillPapanInformasiPermission extends Migration
{
    public function up()
    {
        $db = $this->db;
        ...
        foreach ($adminUsers as $row) {
            $alreadyGranted = $db->table('auth_permissions_users')
                ...
                ->where('permission', 'menu.papan_informasi')
                ->countAllResults() > 0;
            ...
            $db->table('auth_permissions_users')->insert([
                'user_id'    => $row->user_id,
                'permission' => 'menu.papan_informasi',
                'created_at' => $now,
            ]);
        }
    }
